...
class ScreenDivider:
    def __init__(self):
        ...
        self.upper_gray.bind('<Button-3>', self.show_menu)
        
        self.lower_gray.bind('<Button-1>', self.start_drag)
        self.lower_gray.bind('<B1-Motion>', self.drag)
        self.lower_gray.bind('<Button-3>', self.show_menu)
    ...
    def capture_upper_screen(self):
        ...
        try:
            ...
            # 捕获横杠上方的屏幕区域
            bbox = (0, 0, self.screen_width, split_y)
            ...
            # print(f"调试截图已保存: {debug_filename}")
    ...
    def split_screen(self):
        ...
        if not self.is_splitting:
            try:
                ...
                # 绑定滚轮事件到上半部分
                self.upper_canvas.bind('<MouseWheel>', self.block_scroll)
                self.upper_canvas.bind('<Button-4>', self.block_scroll)  # Linux滚轮上
                self.upper_canvas.bind('<Button-5>', self.block_scroll)  # Linux滚轮下
                
                # 绑定鼠标事件，确保优先级
                self.upper_canvas.bind('<Button-1>', self.block_all)
                self.upper_canvas.bind('<Button-2>', self.block_all)
                self.upper_canvas.bind('<Button-3>', self.block_all)
                self.upper_canvas.bind('<Motion>', self.block_all)
                ...
                # messagebox.showinfo("提示", "屏幕已分割！上半部分画面已固定。")
            except Exception as e:
...
def main():
    try:
        ...
        messagebox.showerror("启动错误", f"{app.get_text('error_startup')}{str(e)}")
        sys.exit(1)
